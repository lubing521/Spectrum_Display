...
{
    if(key->key1 == KEY_JUST_PRESSED)   { LED3_TOGGLE(); } // 按键1 刚按下时的动作: LED3取反
    

    if(key->key2 == KEY_JUST_PRESSED)   { LED4_ON();  }    // 按键2 刚按下时的动作: LED4灯亮
    else if(key->key2 == KEY_JUST_POPUP){ LED4_OFF(); }    // 按键2 刚弹出时的动作: LED4灯暗
    
    if(key->key3 == KEY_JUST_PRESSED)   { LED4_ON();  }    // 按键3 
   
    if(key->key4 == KEY_JUST_PRESSED)   { U1_Test_SendString(); } // 按键4 刚按下时的动作: 串口1发送一串字符串
}
